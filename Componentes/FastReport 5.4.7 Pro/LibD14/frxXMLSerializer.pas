...
var
  ...
  begin
    ...
    //THackedObjectReader(FReader.Driver).FBufPos := 0;
    FReaderStream.Position := 0;
    {$ELSE}
    FReader.Position := 0;
    {$ENDIF}
...
{$IFDEF FPC}
      if obj1.ClassName = 'TfrxChartView'  then
      while (THackedObjectReader(FReader.Driver).FBufPos < FReaderStream.Size)  or (FReaderStream.Position < FReaderStream.Size) do
        THackReader(FReader).ReadProperty(Obj1)
      else
      while (THackedObjectReader(FReader.Driver).FBufPos < FReaderStream.Size)  and (FReaderStream.Position < FReaderStream.Size) do
        THackReader(FReader).ReadProperty(Obj1);
