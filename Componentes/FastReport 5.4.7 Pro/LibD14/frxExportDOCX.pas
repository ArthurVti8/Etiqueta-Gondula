...
procedure TfrxDOCXExport.FinishPage(Page: TfrxReportPage; Index: Integer);
  ...
  begin
    ...
    Result := Format('<w:rPr><w:rFonts w:ascii="%s" w:hAnsi="%s" w:cs="%s"/>%s<w:bCs/>%s%s<w:color w:val="#%.6X"/><w:w w:val="105"/>' +
    '<w:sz w:val="%d"/><w:szCs w:val="%d"/>%s<w:vertAlign w:val="%s"/></w:rPr>',
    [f.Name, f.Name, f.Name, BStr(fsBold in f.Style, '<w:b/>'), BStr(fsItalic in f.Style, '<w:i/>'), BStr(fsStrikeOut in f.Style, '<w:strike/>'), RGBSwap(f.Color), f.Size * 2, f.Size * 2, BStr(fsUnderline in f.Style, '<w:u w:val="single"/>'), st]);
  end;
  ...
  function GetSpacedText(const s: string): string; {SAMO}
  begin
    if s = '' then
      Result := ''
    else
  ...
  begin
    ...
    try
      ...
      repeat
        ...
        bEOL := Copy(s, i, Length(sLineBreak)) = sLineBreak;

        if bStyleChanged or bEOL or bEOF then
        begin
          sChunk := GetSpacedText(Copy(s, iPos, i - iPos));
          iPos := i;
          if sChunk <> '' then
            AWriter.Write(['<w:r>', GetFont(AStyle.Font, ASubType), '', sChunk, '</w:r>'], True);//{$IFDEF Delphi12}True{$ELSE}False{$ENDIF});
        end;

        if bEOL then
        begin
          iPos := i + Length(sLineBreak);
          AWriter.Write(['<w:r>', GetFont(SaveFont, SaveSubType), '<w:br/>', '</w:r>'], {$IFDEF Delphi12}True{$ELSE}False{$ENDIF});
        end;
        ...
        if (Obj <> nil) then
          if Obj.IsText then
            pPr := '<w:pPr><w:widowControl w:val="0"/>' +
                   ...
                   '" w:lineRule="exact"/><w:ind w:left="15"/>';
