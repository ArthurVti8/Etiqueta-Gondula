...
begin
  ...
  begin
    ...
    begin
      xi := FPagesItem[StrToInt(FPageCache[0])];
    ...
    begin
      ...
      begin
        ...
        if TfrxPreview(Report.Preview).HasVisibleTabs then
          Application.ActivateHint(Point(X + Report.Preview.ClientOrigin.X, Y + Report.Preview.ClientOrigin.Y + 22))
        else
          Application.ActivateHint(Point(X + Report.Preview.ClientOrigin.X, Y + Report.Preview.ClientOrigin.Y));
      end;
      ...
      if (MouseEvent in [meClick, meDbClick]) and (drill = nil) and (FLastObjectOver <> nil) then
      begin
