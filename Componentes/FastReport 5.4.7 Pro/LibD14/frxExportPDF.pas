...
procedure TfrxPDFExport.WritePDFStream(target: TStream; source: TMemoryStream; id: longint; compressed: Boolean;
    encrypted: Boolean; startingBrackets: Boolean; endingBrackets: Boolean; enableLength2: Boolean);
const
      CR:byte = 10;
var
...
function TfrxPDFExport.StrToHexSp(const SourceStr, Value: WideString; SpaceAdjustment: Extended): AnsiString;
var
  i: integer;
  w: Integer;
  z: Extended;
...
{$ENDIF}
  ...
  begin
    ...
    if not Result then
      Exit;
    FLineWidth := HWidth;
    SpaceAdjustment := pdfFont.LastRemappedStrWidth - HWidth * PDF_DIVIDER * 1000 / textObj.Font.Size;
...
{$ENDIF}
        ...
        if (FCharSpacing <> 0) and (textObj.HAlign <> haBlock) then
        ...
        begin
          ...
          begin
            ...
            RemappedStr := pdfFont.RemapString(Lines[i], textObj.RTLReading);
...
{$IFNDEF OLD_STYLE}
  ...
  begin
    ...
    try
      ...
      if obj is TfrxPictureView then
        begin
          ...
          if (PictObj.Transparent and FTransparency) or PictObj.Center then
            begin
    ...
    if Obj is TfrxPictureView then
      begin
        if PictObj.KeepAspectRatio and not PictObj.Center and PictObj.Stretched then
          begin
