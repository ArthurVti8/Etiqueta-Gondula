...
{$ENDIF}
  ...
  private
    ...
    procedure SplitOnTags( pNode: TfrxXMLItem; obj: TfrxIEMObject;
      line: {$IFNDEF FPC}WideString{$ELSE}string{$ENDIF} );
...
begin
  Result := '';
  s := Str;
  if Copy(s, Length(s) - 1, 4) = {$IFNDEF FPC}#13#10{$ELSE}sLineBreak{$ENDIF} then
...
begin
  ...
  begin
    ...
    if (Obj.Metafile <> nil) and (Obj.Metafile.Width > 0) and (Obj.Metafile.Height > 0) then
      SaveGraphicAs(Obj.Metafile,
      FTempFolder + 'Pictures' + PathDelim + s, PictureType)
    else if (Obj.Image <> nil) then
      SaveGraphicAs(Obj.Image,
      FTempFolder + 'Pictures' + PathDelim +  s, PictureType);
...
//{$ELSE}
  SaveJpeg( FThumbImage.Picture,
    FTempFolder + 'Thumbnails' + PathDelim + 'thumbnail.jpg');
//{$IFEND}
  CreateStyles;
...
{$ENDIF}
  ...
  {$IFNDEF FPC}DeleteFolder(FTempFolder);{$ELSE}DeleteDirectory(FTempFolder,False);{$ENDIF}
end;
