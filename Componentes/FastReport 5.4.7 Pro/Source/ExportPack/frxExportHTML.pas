...
uses frxUtils, {$IFNDEF FPC}frxFileUtils,{$ENDIF}
  frxUnicodeUtils, frxRes, frxrcExports, Math, frxGraphicUtils;
...
var
  ...
  s: {$IFNDEF FPC}WideString{$ELSE}String{$ENDIF};
begin
  s := Str;
...
{$ELSE}
            text := ChangeReturns({$IFNDEF FPC}UTF8Encode{$ENDIF}(TruncReturns(Obj.Memo.Text)));
{$ENDIF}
