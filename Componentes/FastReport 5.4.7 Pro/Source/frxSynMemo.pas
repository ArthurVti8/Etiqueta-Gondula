...
uses
  {$IFNDEF FPC}Windows, Messages, Imm, Types,{$ENDIF}
...
function IsWordChars(s:string):Boolean;
begin
  Result := False;
  if Length(s) = 1 then
  begin
    Result := Char(s[1]) in WordChars;
  end;
end;
{$ENDIF}
...
begin
  Result := {$IFDEF FPC}UTF8Length(LineAt(Index)){$ELSE}Length(LineAt(Index)){$ENDIF};
end;
...
begin
  ...
  for i := 0 to Pos.Y - 2 do
    Result := Result +
    {$IFDEF FPC}UTF8Length(FText[i]){$ELSE}Length(FText[i]){$ENDIF} +
    {$IFDEF FPC}Length(LineBreak){$ELSE}2{$ENDIF};
  Result := Result + Pos.X;
end;
...
begin
  ...
  while i <= Pos do
  {$IFDEF FPC}
    if UTF8Copy(s,i,1)[1] = LineBreak[1] then
  {$ELSE}
...
begin
  ...
  {$IFDEF FPC}
  Result := UTF8Copy(FText.Text, i, GetPlainTextPos(p2) - i + 1);
  {$ELSE}
...
begin
  ...
  if K > 0 then
     UTF8Delete(s, i, k);
  //UTF8Delete(s, i, GetPlainTextPos(p2) - i + 1);
  UTF8Insert(Value, s, i);
  {$ELSE}
  ...
  FText.Text := s;
  p3 := GetPosPlainText(i +
  // 08.10.2015
  {$IFDEF FPC}UTF8Length(Value){$ELSE}Length(Value){$ENDIF});
  //
  FAllowLinesChange := True;
...
{$ELSE}
  {$IFDEF FPC}
    while IsWordChars(UTF8Copy(s,FPos.X,1)) or IsUTF8(UTF8Copy(s,FPos.X,1)) do
  {$ELSE}
...
var
  ...
  begin
    Result := '';
    s := LineAt(FPos.Y - 1);
    s := Trim({$IFDEF FPC}UTF8Copy(s, 1, FPos.X)
    {$ELSE}Copy(s, 1, FPos.X){$ENDIF});
    ...
    i := {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF};
    while i > 1 do
    begin
      Dec(i);
      if ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = '''')
         and not fl2 then
        fl1 := not fl1
      else if ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = '"')
         and not fl1 then
        fl2 := not fl2
      else if not fl1 and not fl2 and
         ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = ')') then
        Inc(fl3)
      else if not fl1 and not fl2 and
         ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = '(') and (fl3 > 0) then
        Dec(fl3)
      else if not fl1 and not fl2 and
         ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = ']') then
        Inc(fl4)
      else if not fl1 and not fl2 and
         ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = '[') and (fl4 > 0) then
        Dec(fl4)
...
{$ELSE}
        if {$IFDEF FPC}UTF8Copy(s,i,1)[1]{$ELSE}s[i]{$ENDIF}
...
{$ELSE}
    while (i <= {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF})
      and ({$IFDEF FPC}UTF8Copy(s,i,1)[1]{$ELSE}s[i]{$ENDIF} in WordChars)
      {$IFDEF FPC}or IsUTf8(UTF8Copy(s,i,1)){$ENDIF} do
{$ENDIF}
      Inc(i);
    s1 := {$IFDEF FPC}UTF8Copy(s, 1, i - 1){$ELSE}Copy(s, 1, i - 1){$ENDIF};
    stepBack := (i <= {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF})
      and ({$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF} = '(');
    if stepBack then
      s1 := s1 + '()';
    SelText := s1;
    if stepBack then
...
{$ENDIF}
        Inc(i);
      s := {$IFDEF FPC}UTF8Copy(s, 1, i - 1){$ELSE}Copy(s, 1, i - 1){$ENDIF};
...
begin
  s := LineAt(FPos.Y - 1);
  {$IFDEF FPC}
  FText[FPos.Y - 1] := UTF8Copy(s, 1, FPos.X - 1);
  FText.Insert(FPos.Y, UTF8Copy(s, FPos.X, FMaxLength));
  {$ELSE}
...
begin
  ...
  begin
    ...
    begin
      ...
      FText.Delete(FPos.Y - 1);
      SetPos({$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} + 1,
        FPos.Y - 1);
      ClearSyntax(FPos.Y);
      DoChange;
    end;
...
{$ELSE}
  {$IFDEF FPC}
  while (i > 0) and not IsWordChars(UTF8Copy(s,i,1))
    and not IsUtf8(UTF8Copy(s,i,1)) do
  {$ELSE}
  while (i > 0) and not (s[i] in WordChars) do
  {$ENDIF}
{$ENDIF}
    if {$IFDEF FPC}UTF8Copy(s,i,1)[1]{$ELSE}s[i]{$ENDIF} = LineBreak[1] then
       break else
      Dec(i);
{$IFDEF Delphi12}
...
{$ELSE}
  {$IFDEF FPC}
  while (i > 0) and IsWordChars(UTF8Copy(s,i,1))
    or IsUTF8(UTF8Copy(s,i,1)) do
  {$ELSE}
...
{$ELSE}
  {$IFDEF FPC}
  while (i < UTF8Length(s)) and (IsWordChars(UTF8Copy(s,i,1))
    or IsUTf8(UTF8Copy(s,i,1)))  do
  {$ELSE}
...
{$ELSE}
  {$IFDEF FPC}
  while (i < UTF8Length(s))  and not IsWordChars(UTF8Copy(s,i,1))
    and not IsUtf8(UTF8Copy(s,i,1)) do
  {$ELSE}
...
{$ENDIF}
    if {$IFDEF FPC}UTF8Copy(s,i,1)[1]{$ELSE}s[i]{$ENDIF} = LineBreak[1] then
    begin
      while (i > 1) and ({$IFDEF FPC}UTF8Copy(s,i-1,1){$ELSE}s[i - 1]{$ENDIF} = ' ') do
        Dec(i);
      break;
    end
    else
...
{$ENDIF}
  begin
    if FIsMonoType then
    begin
      Canvas.FillRect(Rect(x, y, x +
      {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} * FCharWidth, y +
      FCharHeight));
...
{$ELSE}
      for i := 1 to {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} do
        ...
        {$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF});
      MoveTo(x +
        {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} * FCharWidth, y);
{$ENDIF}
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if j > {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} then
           break;
        ...
        begin
          SetAttr(a, i);
          MyTextOut(PenPos.X, PenPos.Y,
            {$IFDEF FPC}UTF8Copy(FText[i], j1, j - j1){$ELSE}Copy(FText[i], j1, j - j1){$ENDIF});
          a := a1;
          j1 := j;
        end;
      end;
      ...
      SetAttr(a, i);
      MyTextOut(PenPos.X, PenPos.Y,
        {$IFDEF FPC}UTF8Copy(s, j1, FMaxLength){$ELSE}Copy(s, j1, FMaxLength){$ENDIF});
      if (caBlock in GetCharAttr(Point(1, i + 1))) or (i = FActiveLine - 1) then
        MyTextOut(PenPos.X, PenPos.Y, Pad(FWindowSize.X -
        {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} -
        FOffset.X + 3));
...
begin
  ...
  begin
    ...
    if SearchFrom = 0 then
      SearchFrom := 1;
    {$IFDEF FPC}
    s := UTF8Copy(s, SearchFrom, UTF8Length(s) - SearchFrom + 1);
    {$ELSE}
    ...
    begin
      {$IFDEF FPC}
      i := UTF8Pos(UTF8UpperCase(SearchText), UTF8UpperCase(s));
      {$ELSE}
  ...
  begin
    ...
    {$IFDEF FPC}
    FSelEnd := Point(FSelStart.X + UTF8Length(SearchText), FSelStart.Y);
    Inc(SearchFrom, UTF8Length(SearchText));
    {$ELSE}
